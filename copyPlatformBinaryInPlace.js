...
copyBinary("exe/bs-react-intl-ppx-" + platform + "-" + arch + ".exe", "ppx");

function copyBinary(filename, destFilename) {
  var supported = fs.existsSync(filename);

  if (!supported) {
    console.error("bs-react-intl-ppx does not support this platform :(");
    console.error("");
    console.error(
      "bs-react-intl-ppx comes prepacked as built binaries to avoid large"
    );
    console.error("dependencies at build-time.");
    console.error("");
    console.error("If you want bs-react-intl-ppx to support this platform natively,");
    console.error(
